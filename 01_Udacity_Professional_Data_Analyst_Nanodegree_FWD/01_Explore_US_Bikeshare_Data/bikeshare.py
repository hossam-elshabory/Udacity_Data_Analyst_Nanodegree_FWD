...
def get_filters() -> tuple[str, str, str]:
    """_summary_ : Asks user to specify a city, month, and day to analyze.

    Returns
    -------
    tuple[str, str, str]
        _description_ : A tuple of strings containing the city, month and day.
    """
    ...
    while month not in months:
        month = input(
            f"Please choose a month to filter by:\n( {' - '.join(months)} )\n"
        ).lower()
    ...
    while day not in week_days:
        day = input(
            f"Please enter a day to filter by:\n( {' - '.join(week_days)} )\n"
        ).lower()
...
def load_data(city: str, month: str, day: str) -> DataFrame:
    """_summary_ : Loads data for the specified city and filters by month and day if applicable.

    Parameters
    ----------
    city : str
        _description_ : Name of the city to analyze. 
    month : str
        _description_ : Name of the month to filter by, or "all" to apply no month filter.
    day : str
        _description_ : Name of the day of week to filter by, or "all" to apply no day filter.

    Returns
    -------
    DataFrame
        _description_ : Pandas DataFrame containing city data filtered by month and day
    """
...
def trip_duration_stats(df: DataFrame) -> None:
    ...
    print(
        f"The Total Travel Time is: {total_travel_time_hours} Hours, {total_travel_time_mints} Minutes"
    )
...
def display_data(df: DataFrame) -> None:
    ...
    while loop_flag:
        response = input(
            "Do you wish to view the raw data? type 'yes' or 'no' \n"
        ).lower()
        if response == "yes":
